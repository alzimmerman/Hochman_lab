...
function [amp,dur,AHPdur,AHPmag,Vth,spikes2]=findspikes(time,Vm,clampon,clampoff)
...
if (isempty(k)~=1)
    ...
    for (i=1:Cth)
        ...
        if (isempty(AP)~=1)
            dur(i)=length(AP)*time(2);  %defines duration as time spent over threshold
            [Vmax(i),samplemax(i)]=max(Vm(spikes2(i):(endpoint-1)));
            [Vmin(i),samplemin(i)]=min(Vm(spikes2(i):(endpoint-1)));
            smax(i)=spikes2(i)+samplemax(i);
            smin(i)=spikes2(i)+samplemin(i);
        
            % defines first and last sample when signal is below threshold for preceding spike
            ftemp(i)= find(Vm(smax(i):endpoint) < Vth(i),1,'first')  ;
            ltemp(i)= find(Vm(smax(i):endpoint) < Vth(i),1,'last')  ;  
            first(i)=ftemp(i)+smax(i);
            last(i)=ltemp(i)+smax(i);
            
            if (isempty(first)~=1)
                AHPdur(i)=(last(i)-first(i))./Fs;
                amp(i)=Vmax(i)-Vth(i);
