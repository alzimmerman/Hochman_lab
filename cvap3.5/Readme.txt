Help manual of Cluster Validity Analysis Platform (CVAP) (Version 3.5)


Your comments are welcome and are helpful to the improvements of this tool.
Mr. WANG Kaijun, E-mail: wangkjun@yahoo.com

(1) Contents of CVAP
Purpose
    The evaluation of clustering results is important in cluster analysis of gene expression data, and it is validity indices that are usually used to evaluate clustering results. To supply a visual analysis environment to facilitate cluster validation, we developed cluster validity analysis platform (CVAP) and implemented most of the validity indices listed below. The CVAP based on a graphical user interface (GUI) is a convenient working environment for cluster validation, integrating 18 validity indices and several clustering algorithms (PAM, K-means, hierarchical, SOM and etc.).
...
Note 1: To start CVAP, type "mainCVAP" in Matlab command window for Matlab version 7.x, but use "mainCVAP6" for Matlab version 6.5. The CVAP and its GUI were tested under Matlab 7.2 and 6.5.
(Please do not double click "mainCVAP.fig" to attempt to start CVAP, since "mainCVAP.fig" is a figure file, not Matlab codes.)
...
and  you should choose "All columns/rows are data" in the pop-up menu "Position".  
    ...
    When true class labels (total two classes) are available, put them in the first column of the data file:
2	5.1	3.5	1.4	0.2
2	4.9	3	1.4	0.2
1	4.7	3.2	1.3	0.2
1	1.7	1.2	1.3	0.2
and  you should choose "Class labels in first column/row" (default) in the pop-up menu "Position", and not change "Clustering Rows" (default) in Option menu.
    ...
    For example, for the above example the solution file "mysolution.txt" (you should cluster the data into 1, 2 , 3 and 4 clusters respectively to yield the solution file) looks like:
1	2	3	4
1	2	2	1
1	2	2	2
1	2	3	3
1	1	1	4
The first row indicates numbers of clusters (1, 2 , 3 and 4 clusters), column 3 (except the first row) means the 3-class labels of the data that are clustered to 3 clusters, and etc.
    When true class labels are available, they are in the first column and the first element of this column is 0 to indicate true lables. The file looks like:
0	2	3	4
2	2	2	1
2	2	2	2
1	2	3	3
...
Note 1: PAM codes are from LIBRA (http://wis.kuleuven.be/stat/robust/LIBRA.html),  hierarchical clustering codes are from Clustering Toolbox by David Corney (www.cs.ucl.ac.uk/staff/D.Corney/index.htm), SOM codes are from Netlab neural network software (www.ncrg.aston.ac.uk/netlab/), and Nuero-Gas codes are from SOM Toolbox (by Esa Alhoniemi and etc. somtlbx@mail.cis.hut.fi). The authors of these codes hold copyrights. The K-means routine is from Statistics Toolbox of Matlab.

Note 2: Initialization of K-means is to select K centroids from data at random, for other choices, refer to valid_clusteringAlgs.m (row 31) of CVAP and kmeans.m of Matlab.

Note 3: Initialization and training methods of SOM are similar to the ones in "demsom1.m" of Netlab except adaptive iterations and initial neighborhood size, see "som_netlab.m" of CVAP for detail.

Note 4: A new clustering algorithm may be added in last 4 rows of "valid_clusteringAlgs.m" if you are familiar to code making, and then change "ialg >= 8" to "ialg > 8" in row 2 of "valid_runclustering.m". It is to select "New algorithm" in pop-up menu "Algorithm" when using this added algorithm.
...
Demo data set "leuk72_3k" is a Leukaemia dataset with selected features (39 genes), it has 72 tissue samples in 3 close well-separated classes, and true class labels are in the first column of data file "leuk72_3k.txt".

To start CVAP, use "mainCVAP" in Matlab command window for Matlab version 7.x (but "mainCVAP6" for version 6.5). The clustering process should be performed before cluster validation processes, but no clustering process is needed for a solution file. 
<1> Clustering process
  # Loading a data file
  Click "Load Demo Data" in File menu bar, and then "Running state: Default data set 'leuk72_3k' is loaded, ..." appears in the running state window.
  You may click "Load Data File" to load a new data file.
...
iii) Comparison analysis between several algorithms
  ...
  Now the Silhouette values corresponding to PAM, k-means and etc. are ready for further analyses (refer to above-mentioned references for real examples).

---------------------------------------------------------------------------------------------------
This free software is distributed under LGPL license.  (see Copyright.txt)
